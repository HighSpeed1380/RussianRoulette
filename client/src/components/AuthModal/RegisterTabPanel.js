...
const validSchema = yup.object().shape({
  ...
  password: yup
    .string()
    .min(3, "Password should be 3 characters at minimum")
...
export default function RegisterTabPanel() {
  ...
  const Register = async (values) => {
    ...
    console.log("sign up save :::", JSON.stringify(generatedAddress));
    const { username, email, password, role, level, balance } = values;
    await signup({
      username,
      email,
  ...
  return (
    <Stack spacing={2}>
      ...
      <LoginButton onClick={formik.handleSubmit}>PLAY NOW</LoginButton>
